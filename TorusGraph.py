...
class TorusGraph():
    ...
    def add_blocked_edges_ttb_left(self, placed_edge):
        ...
        for top_left in top_left_blocked:
            top_left_copy = top_left_blocked.copy()
            top_left_copy.remove(top_left)
            for left_2 in top_left_copy:
                if top_left < left_2:
                    self.remove_typed_edge(((top_left, left_2), "topWrap"))
                else:
                    self.remove_typed_edge(((left_2, top_left), "topWrap"))
        ...
        for bottom_right in bottom_right_blocked:
            bottom_right_copy = bottom_right_blocked.copy()
            bottom_right_copy.remove(bottom_right)
            for right_2 in bottom_right_copy:
                if bottom_right < right_2:
                    self.remove_typed_edge(((bottom_right, right_2), "bottomWrap"))
                else:
                    self.remove_typed_edge(((right_2, bottom_right), "bottomWrap"))
        ...
        for top_left in top_left_blocked:
            for bottom_right in bottom_right_blocked:
                if top_left < bottom_right:
                    self.remove_typed_edge(((top_left, bottom_right), "ttbLeft"))
                else:
                    self.remove_typed_edge(((bottom_right, top_left), "bttRight"))

        top_outside = [self.top_spine[i] for i in range(smaller + 1, self.verts)]
        for to1 in top_outside:
            to_copy = top_outside.copy()
            to_copy.remove(to1)
            for to2 in to_copy:
                if to1 < to2:
                    self.remove_typed_edge(((to1, to2), "topWrap"))
                else:
                    self.remove_typed_edge(((to2, to1), "topWrap"))

        bottom_outside = [self.bottom_spine[i] for i in range(0, larger)]
        for bo1 in bottom_outside:
            bo_copy = bottom_outside.copy()
            bo_copy.remove(bo1)
            for bo2 in bo_copy:
                if bo1 < bo2:
                    self.remove_typed_edge(((bo1, bo2), "bototmWrap"))
                else:
                    self.remove_typed_edge(((bo2, bo1), "bottomWrap"))

        for to in top_outside:
            for bo in bottom_outside:
                if to < bo:
                    self.remove_typed_edge(((to, bo), "ttbLeft"))
                    self.remove_typed_edge(((to, bo), "ttbRight"))
                else:
                    self.remove_typed_edge(((bo, to), "bttRight"))
                    self.remove_typed_edge(((bo, to), "bttLeft"))
    ...
    def add_blocked_edges_ttb_right(self, placed_edge):
        ...
        for top_right in top_right_blocked:
            top_right_copy = top_right_blocked.copy()
            top_right_copy.remove(top_right)
            for right_2 in top_right_copy:
                if top_right < right_2:
                    self.remove_typed_edge(((top_right, right_2), "topWrap"))
                else:
                    self.remove_typed_edge(((right_2, top_right), "topWrap"))
        ...
        for bottom_left in bottom_left_blocked:
            bottom_left_copy = bottom_left_blocked.copy()
            bottom_left_copy.remove(bottom_left)
            for left_2 in bottom_left_copy:
                if bottom_left < left_2:
                    self.remove_typed_edge(((bottom_left, left_2), "bottomWrap"))
                else:
                    self.remove_typed_edge(((left_2, bottom_left), "bottomWrap"))
        ...
        for top_right in top_right_blocked:
            for bottom_left in bottom_left_blocked:
                if top_right < bottom_left:
                    self.remove_typed_edge(((top_right, bottom_left), "ttbRight"))
                else:
                    self.remove_typed_edge(((bottom_left, top_right), "bttLeft"))
        
        top_outside = [self.top_spine[i] for i in range(0, smaller)]
        for to1 in top_outside:
            to_copy = top_outside.copy()
            to_copy.remove(to1)
            for to2 in to_copy:
        ...
        bottom_outside = [self.bottom_spine[i] for i in range(larger + 1, self.verts)]
        for bo1 in bottom_outside:
            bo_copy = bottom_outside.copy()
            bo_copy.remove(bo1)
            for bo2 in bo_copy:
    ...
    def add_blocked_edges_btt_left(self, placed_edge):
        ...
        top_outside = [self.top_spine[i] for i in range(0, larger)]
        for to1 in top_outside:
            to_copy = top_outside.copy()
            to_copy.remove(to1)
            for to2 in to_copy:
        ...
        bottom_outside = [self.bottom_spine[i] for i in range(smaller + 1, self.verts)]
        for bo1 in bottom_outside:
            bo_copy = bottom_outside.copy()
            bo_copy.remove(bo1)
            for bo2 in bo_copy:
    ...
    def add_blocked_edges_btt_right(self, placed_edge):
        ...
        top_outside = [self.top_spine[i] for i in range(larger + 1, self.verts)]
        for to1 in top_outside:
            to_copy = top_outside.copy()
            to_copy.remove(to1)
            for to2 in to_copy:
        ...
        bottom_outside = [self.bottom_spine[i] for i in range(0, smaller)]
        for bo1 in bottom_outside:
            bo_copy = bottom_outside.copy()
            bo_copy.remove(bo1)
            for bo2 in bo_copy:
